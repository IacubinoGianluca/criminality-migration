...
```
Per la stessa ragione di prima andremo a verificare se persiste il medesimo problema riscontrato per L'ISO del Regno Unito nei dataset **assault**.
...
sexViolence %>% count(Year, ISO) %>% filter(n >1)
```
Come fatto in precedenza andiamo a visualizzare gli stati che presentano una maggior concentrazione di questo crimine.
```{R}
...
Come per gli altri tipi di crimine andiamo a graficare nella mappa i paesi con più immigrazione e con più episodi di violenza sessuale.
